...
def _load_cluster_batch(cfg: DictConfig, cluster_id: str, device: torch.device) -> dict:
    datamodule = RNAConformerDataModule(cfg.data_cfg)
    datamodule.setup()

    datasets = [
        datamodule.val_ensemble_dataset,
        datamodule.val_single_dataset,
    ...
    for dataset in datasets:
        ...
        if dataset.ensemble_as_cluster:
            if cluster_id in dataset.cluster_names:
                idx = dataset.cluster_names.index(cluster_id)
                batch = conformer_collate([dataset[idx]])
                break
        else:
            idx = next(
                (
                    ...
                    if str(record["cluster_name"]) == cluster_id
                ),
                None,
            )
            if idx is not None:
        ...
        raise FileNotFoundError(f"Cluster not found in preprocessed validation/test data: {cluster_id}")
...
def run(cfg: DictConfig) -> None:
    ...
    batch = _load_cluster_batch(cfg, cluster_id, device)
