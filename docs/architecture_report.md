...
| 3 | SE(3) Stochastic Bridge with 残基级 $\sigma = \bar\sigma\sqrt{\nu}$ | Sequence-only `FlexibilityNet` + B-factor 监督 | SDE |

三级通过 `stochastic_bridge.level` 字段切换，共享同一套 FlowModel 漂移网络和损失形式 —— **各级的唯一差别是前向过程 (corruption) 和 $\sigma$ 的来源**。Level 3 在 $\nu \equiv 1$ 的初始化下**严格退化**为 Level 2，可以从 Level 2 checkpoint 热启动。
...
$$u^x(t) = \frac{x_1 - x_t}{1-t}, \qquad u^\omega(t) = \frac{1}{1-t}\mathrm{Log}(R_t^\top R_1)^\vee$$

唯一的改动在 $R_t$（现在从 BB 采样而非沿测地线）。Conditional Flow Matching 理论保证，只要 conditional 采样分布正确，训练就能拟合对应的 marginal velocity field —— 训练代码、损失、网络、per-modality 权重全部复用 Level 1。

### 2.6 Corruption 实现（`interpolant.py`）

Level 2 的代码改动：(a) `_corrupt_rotmats_bridge` 在切空间做 BB 采样并 push-forward 到 SO(3)；(b) `_corrupt_trans_bridge` 在线性均值上加 BB 噪声，**并在 resolved residues 上做 post-multiply 残基均值重心化**，保留 centered-Gaussian translation prior。两个函数都接受可选的 `nu`（Level 3 用），`nu=None` 时退化为常数 $\sigma$ 的 Level 2 桥。
...
def _corrupt_trans_bridge(self, trans_1, t, res_mask, sigma_x, nu=None):
    ...
    # Post-multiply re-center across resolved residues per sample so that
    # sum_{i in R} eta_centered_i = 0 for any (heteroscedastic) nu, restoring
    # the centered-Gaussian translation prior the L1 drift was trained against.
    res_mask_f = res_mask.to(eta.dtype)
    ...
    trans_t = mean + std[..., None, None] * eta_centered
...
$\sigma_x, \sigma_\omega$ 直接从 `self._bridge_cfg.bridge` 读取，**不再依赖 BridgeWidthNet**。`_trans_euler_maruyama_step` / `_rots_euler_maruyama_step` 接受 per-residue 形状的 `a_trans` / `a_rots` 张量；`Interpolant.sample_bridge(num_batch, num_res, model, context=None, nu=None)` 内部把标量 $(\bar\sigma_x, \bar\sigma_\omega)$ 广播成 `[B, N]` 的张量后送进这两个 helper（Level 2 时 `nu=None` 等同于 broadcast 1，Level 3 时按 $\bar\sigma\sqrt{\nu_i}$ 形成 per-residue 步长）。这是 paper §4.2 描述的 Euler-Maruyama 实现。
...
1. **Corruption + inference 层面严格嵌套 (L2 ⊂ L3)**：令 $\nu \equiv 1$（对所有残基），Level 3 的 **corruption** 张量 $(x_t, R_t)$、**velocity target** 与 **inference Euler-Maruyama 单步** 都和 Level 2 逐 bit 相同（不是"approximately similar"，是严格退化）。**注意**：完整的 Level 3 训练目标是 $\mathcal{L}_{\text{level3}} = \mathcal{L}_{\text{bridge}} + \lambda_B \mathcal{L}_B + \lambda_\nu \mathcal{L}_\nu$，比 Level 2 多两项；这两项的梯度通过 `nu.detach()` stop-grad firewall 只回到 $\psi$，不回到 FlowModel，所以 FlowModel 的训练动力学和 Level 2 一致。这是"严格嵌套于 corruption + inference 层面，objective 层面通过 stop-grad 实现等价"的表述，而不是"objective 也逐 bit 相同"。
...
给定 per-residue 条件特征 $s \in \mathbb{R}^{c_\text{single}}$（直接用 dataset 的 `single_embedding`，不和 FlowModel backbone 共享），定义一个小 MLP head $g_\psi: \mathbb{R}^{c_\text{single}} \to \mathbb{R}$ 和 per-sample 归一化：

$$a = \mathrm{softplus}(g_\psi(s)) + \varepsilon, \qquad \nu = \frac{a}{\frac{1}{L}\sum_{k=1}^{L} a_k}$$

其中 $\varepsilon > 0$ 是数值稳定常数，$L$ 是当前样本的有效 nucleotide 数 (`is_na_residue_mask.sum()`，即 sequence mask，**不**是 res_mask)；$\sum_{k=1}^L a_k$ 中的 $k$ 是遍历残基的 dummy index。

> **Mask contract（关键，与 paper §4.1 一致）**：head 内部的 mean-1 归一化用 **sequence mask** (`is_na_residue_mask`)，覆盖所有 padding-外的真实 nucleotide，不论结构是否 resolved。`res_mask`（resolved-structure 子集）只在 (a) `L_bridge` reduction 和 (b) `L_B` 的 valid-B gate 里出现。这样 train/val/predict 三处的 $\nu$ normalization domain 完全一致——training 直接 forward `seq_mask = batch["is_na_residue_mask"].float()`；validation/`predict_step` 在按 NA mask 剥离 padding 之后传 all-ones mask，此时 all-ones mask **就是** sequence mask，自动一致。代码上 `flow_module.training_step` 已经按这个 contract 调用。
...
Head 实现：2 层 MLP，最后一层 `Linear` **零初始化 weight**、bias 设 `log(e-1) ≈ 0.5413`（这样 softplus 输出 1），保证 $\nu \equiv 1$ 严格成立。
...
$$a^\star_t = J_r(\Omega_t)\cdot\frac{\omega - \Omega_t}{1-t}$$

**代码实现**复用 Level 2 的简化形式（与 Level 1 一字不差）：

$$u^x(t) = \frac{x_1 - x_t}{1-t}, \qquad u^\omega(t) = \frac{\mathrm{Log}(R_t^\top R_1)^\vee}{1-t}$$
...
**归一化 B-factor target**（per-sample，消除 resolution/refinement scale 差异）。写第 $k$ 个残基的归一化 B-factor $b_k$：

$$b_k = \frac{B_k}{\frac{1}{L}\sum_{j=1}^{L} B_j}, \qquad \widehat{b}_k = \frac{\kappa\,\nu_k}{\frac{1}{L}\sum_j \kappa\,\nu_j} = \frac{\nu_k}{\frac{1}{L}\sum_j \nu_j} = \nu_k$$
...
**关于 $\hat b_i = \nu^{(B)}_i$ 的精度**：在 constant-$\kappa$ 假设 + post-multiply centered translation bridge 下，这个 identity **不是** strictly exact —— 而是 $m_R$ 的 asymptotic limit，带 $O(1/m_R)$ 修正。修正系数在 `ensemble_dataset2` 长度（$m_R \in [30, 300]$）上 $\le 3.4\%$，远低于晶体学 B-factor 的 refinement noise floor ($\pm 10\text{--}20\%$)。完整推导见 paper Appendix C "Centered-bridge B-factor proxy"。

代码上：bridge corruption 仍用 sequence-mask 归一化的 $\nu$；`L_B` 内部的双重归一化 (`nu_validB`) 是纯 loss-internal 操作，不影响 corruption。$c$ 和 $\kappa$ 在 loss 里不出现，只用于 writeup 的物理动机。
...
**Stage 1 — $\psi$-only warmup**（可选）：冻结 FlowModel（`configure_optimizers` 把 `model.parameters()` 设 `requires_grad=False`），只训 FlexibilityNet，目标 $\mathcal{L}_B + \lambda_\nu \mathcal{L}_\nu$。$\mathcal{L}_{\text{bridge}}$ 此时仍然被 *计算* 并 log，但因为 FlowModel 参数全部 frozen，它对 $\theta$ 的 gradient 是 inert（zero update），因此实际只有 $\psi$ 在更新。让 $\nu$ 先学会合理的 residue profile 再解冻 backbone。通过 `stochastic_bridge.training_phase: psi_only` 开启。

**Stage 2 — joint fine-tune（"full" phase）**：解冻所有参数，目标 $\mathcal{L}_{\text{level3}} = \mathcal{L}_{\text{bridge}} + \lambda_B\mathcal{L}_B + \lambda_\nu\mathcal{L}_\nu$。通过 `stochastic_bridge.training_phase: full` 开启。**注意**：code 里 `_training_phase` 字段只接受 `"full"` 或 `"psi_only"` 两个值（`flow_module.py` 的 `__init__`），没有 `joint_stopgrad` 这个名字 —— 早期文档草案曾用过 `joint_stopgrad`，正式实现统一为 `full`。
...
```python
# seq_mask = is_na_residue_mask（包含所有真实 nucleotide，padding 排除）
seq_mask = batch["is_na_residue_mask"].to(batch["single_embedding"].dtype)
nu_raw = self.flexibility_net(batch["single_embedding"], seq_mask)  # grad → ψ
...
对给定 sequence，先一次性算出 $\nu = \text{FlexibilityNet}(s)$（no_grad，输出所有残基的 $\nu$ 值），然后每个 residue 用各自的宽度做 Euler-Maruyama（下式中的 $k$ 是离散化时间步，与残基下标无关）：

$$x_{k+1} = x_k + \Delta t \cdot u_\theta^x + \bar\sigma_x\sqrt{\nu}\sqrt{\Delta t}\cdot\xi$$

$$R_{k+1} = R_k\cdot\mathrm{Exp}\!\left(\widehat{\Delta t\cdot u_\theta^\omega + \bar\sigma_\omega\sqrt{\nu}\sqrt{\Delta t}\cdot\zeta}\right)$$

Rigid residue ($\nu < 1$) 自动使用更小噪声，flexible residue ($\nu > 1$) 自动使用更大噪声。若需 probability-flow ODE，把所有噪声项置零即可。
...
- **NMR / cryo-EM fallback（三层 sample-level gate）**：现代 PDB 里 NMR 结构的 b-factor 通常全为 0；某些 cryo-EM 结构则有 refined-but-uniform 的 B-factor。Level 3 在 `L_B` 计算前做 per-residue + per-sample gating（对应 code `_compute_flex_losses`）：
  - **Residue 级**：只对 `res_mask == 1 AND b_factors > min_valid_bfactor` 的 residue 计 loss（参数 `bfactor.min_valid_bfactor`）
...
- **缺失环（missing-loop）残基**：`res_mask = 0` 的位置在训练中不计入 `L_B`，但 inference 时 head 仍按 sequence-only 输出 $\nu_i$（因为 head 只消费 `single_embedding`）。这是"sequence-only extrapolation"，不是 supervised interpolation，需要在 limitations 里说清楚
...
### 3.10 与 Level 2 的嵌套关系（corruption + inference 层面 strict，objective 层面 stop-grad 等价）
...
- 此 init 状态下，**inference Euler-Maruyama 单步** 与 Level 2 bit-identical
- 此 init 状态下 $\log \nu = 0$，$\mathcal{L}_\nu = 0$
...
> **总结**：strict bit-for-bit 嵌套是 *corruption-level* 和 *inference-level* 的；objective-level 通过 stop-grad firewall 实现 effective-gradient 等价，而不是逐 bit 相同。这是 paper 的诚实表述。
...
| FlexibilityNet（Level 3 新增） | ~50,177 | LayerNorm(384) + Linear(384→128) + SiLU + Linear(128→1) |
| **Level 3 总计** | ~14,298,687 | +0.35% over Level 2 |

Level 2 相对 Level 1 **没有新增任何参数**，全部随机性由 BB 噪声引入。Level 3 新增的 FlexibilityNet 只有约 50K 参数（<0.4%），且在初始化时严格等价于 Level 2（$\nu_i \equiv 1$）。
