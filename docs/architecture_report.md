...
1. **严格嵌套 (L2 ⊂ L3)**：令 $\nu \equiv 1$（对所有残基），Level 3 的 corruption、velocity target、loss 与 Level 2 逐 bit 相同。不是"approximately similar"，是严格退化。
...
其中 $\varepsilon > 0$ 是数值稳定常数，$L$ 是当前样本的有效 residue 数 (`res_mask.sum()`)；$\sum_{k=1}^L a_k$ 中的 $k$ 是遍历残基的 dummy index。
...
$$\mathcal{L}_B = \frac{1}{L}\sum_{k=1}^{L}\big(\log\nu_k - \log b_k\big)^2$$

干净到一行 —— `L_B` 本质上是 `log ν` 去拟合 `log b`。$c$ 和 $\kappa$ 在 loss 里不出现，只用于 writeup 的物理动机。
...
- $\mathcal{L}_\nu = \frac{1}{L}\sum_{k=1}^{L} (\log\nu_k)^2$ = 很弱的 $\log$-space 正则，防止 $\nu$ 过度尖锐；因为 $\nu$ 已经 mean-1 归一化，这项也自动中心化
...
- $\psi$ 最后一层 weight = 0 $\Rightarrow$ $a$ 在所有残基上取同一常数 $\Rightarrow$ $\nu \equiv 1$ $\Rightarrow$ bridge corruption 与 Level 2 完全相同
- 在此 init 状态下，$\log \nu = 0$，$\mathcal{L}_\nu = 0$
- $\mathcal{L}_B$ 在此 init 状态下 $= \frac{1}{L}\sum_{k=1}^{L}(\log b_k)^2$，只驱动 $\psi$，不影响 FlowModel
