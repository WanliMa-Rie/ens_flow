...
Level 2 的代码改动：(a) `_corrupt_rotmats_bridge` 在切空间做 BB 采样并 push-forward 到 SO(3)；(b) `_corrupt_trans_bridge` 在线性均值上加 BB 噪声。两个函数都接受可选的 `nu`（Level 3 用），`nu=None` 时退化为常数 $\sigma$ 的 Level 2 桥。平动噪声先按 per-residue 独立方式注入，再在 resolved residues 上做 mask-aware CoM 去均值；这与预处理和推断端的零重心 gauge 对齐。
...
def _corrupt_trans_bridge(self, trans_1, t, res_mask, sigma_x, nu=None):
    ...
    trans_t = _center_trans_masked(trans_t, res_mask)
    return trans_t

def _corrupt_rotmats_bridge(self, rotmats_1, t, res_mask, sigma_omega, nu=None):
    rotmats_0 = self._sample_rotmats_0(rotmats_1, res_mask)        # Haar-uniform SO(3)
