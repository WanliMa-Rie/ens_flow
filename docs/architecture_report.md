...
#### Bridge-consistent stochastic sampler

纯 ODE 采样在实践中会出现 mode collapse：由于 drift 是确定性的，rollout 的多样性完全由初始状态 $(x_0, R_0)$ 的采样决定，而 FlowModel 的 SFM-corrected endpoint predictor 在中段 $t$ 就已经收敛到少数几个 attractor。为恢复 ensemble coverage，在 drift 之上注入**训练带宽形状一致**的高斯探索噪声：
...
其中 $\tilde\xi^x$ 是跨 resolved residues 去均值后的高斯噪声（对应 CoM 约束）。关键性质：

- **Bridge envelope 匹配**：$\alpha_i(t)$ 的 $\sqrt{t(1-t)}$ 形状与训练 corruption 严格一致，噪声在两端自动消失；最后一步自动 snap 到 $\hat x_{1|t}, \hat R_{1|t}$。
- **严格嵌套**：$\tau = 0$ 退化为 ODE（分支内部不消耗任何 `torch.randn`，bit-identical 恢复 Level 1）；$\nu_i \equiv 1$ 退化为 homoscedastic Level 2 sampler。
...
- **解释**：这是建立在 flow drift 之上的 "bridge-shaped annealed Langevin"，不是对 forward bridge SDE 的逐时刻精确 reverse，但与训练 corruption 的方差谱在每单位时间上严格对齐，因此是 "bridge-consistent" 的务实设计。

#### 实现备注

- `Interpolant.sample(..., tau, nu)` 单一入口，`tau=0` 走原 ODE 分支，`tau>0` 走 `_bridge_sde_step`。
- 所有随机数都放在 `else` 分支内，确保 `tau=0.0` 与旧版 ODE 输出 bit-identical。
- `FlexibilityNet` 只在 Level 3 checkpoint 且 `tau>0` 时调用，缓存 $\nu$ 一次，不进入逐步循环。
...
### 3.8 Inference：ODE（与 Level 1-2 共享）

Level 3 的推断与 Level 1-2 完全相同——纯 ODE 积分，$\nu$ **不显式进入推断公式**：

$$x_{k+1} = x_k + \Delta t \cdot \frac{\hat x_{1|t_k} - x_k}{1-t_k}$$

$$R_{k+1} = R_k\cdot\mathrm{Exp}\!\left(\widehat{\Delta t \cdot s_k \cdot \mathrm{Log}(R_k^\top \hat R_{1|t_k})^\vee}\right)$$

**$\nu$ 的作用路径**：$\nu$ 通过训练端间接影响推断输出——

1. **训练时**：$\nu$ 缩放 bridge corruption 的 per-residue 噪声（$\bar\sigma\sqrt{\nu}$），使高 $\nu$ 残基在训练中看到更大扰动
2. **SFM loss**：bridge noise $\delta$ 包含 $\nu$ 的缩放（§3.4），修正项 $\delta/(2t)$ 自动按残基调制
3. **学到的 velocity field**：FlowModel 在 per-residue 异质的噪声分布上训练，隐式学到了残基级的 flexibility 信息
4. **推断时**：ODE 积分学到的 velocity field，$\nu$ 的影响已编码在 $v_\theta$ 中

这一设计与 §2.8 的原则一致：SFM 训练把随机性编码进速度场，推断端只需 ODE 积分。
